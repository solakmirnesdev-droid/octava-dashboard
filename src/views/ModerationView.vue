...
<template>
  <section>
    <header class="mb-5 flex flex-wrap items-center gap-3">
      ...
      <div class="flex overflow-hidden rounded border border-black/15 text-sm">
        <button
          v-for="t in [{ k: 'reviews', l: 'Recenzije' }, { k: 'comments', l: 'Komentari' }]" :key="t.k"
          class="px-3 py-1.5"
          :class="tab === t.k ? 'bg-accent text-white' : 'bg-white hover:text-accent'"
          @click="switchTo(t.k)"
        >{{ t.l }}</button>
      </div>
    ...
    <p class="mb-3 text-sm text-black/45">{{ total }} zapisa</p>
    ...
    <ul v-else class="space-y-2">
      <li v-for="row in items" :key="row._id" class="rounded border border-black/10 bg-white px-4 py-3">
        <div class="flex flex-wrap items-baseline gap-x-2 gap-y-1 text-sm">
          <span class="font-medium">{{ row.user?.username || 'nepoznat' }}</span>
          <span class="text-black/40">{{ row.user?.email }}</span>
          <span v-if="row.song" class="text-black/50">· {{ row.song.title }}</span>
          <span class="text-xs text-black/35">· {{ when(row.createdAt) }}</span>

          <span
            v-if="row.status !== 'published'"
            class="rounded bg-black/5 px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-black/50"
          >{{ row.status === 'hidden' ? 'sakriveno' : 'uklonio autor' }}</span>
        </div>

        <p class="mt-2 whitespace-pre-wrap text-sm text-black/75">{{ row.body }}</p>

        <p v-if="row.status === 'hidden'" class="mt-2 text-xs text-black/45">
        ...
        <div class="mt-3 flex gap-2">
          <button
            v-if="row.status === 'published'"
            class="flex items-center gap-1.5 rounded border border-black/15 px-2.5 py-1 text-sm hover:border-rose-400 hover:text-rose-700"
            @click="askHide(row)"
          ><IconHide /> Sakrij</button>

          <button
            v-else-if="row.status === 'hidden'"
            class="flex items-center gap-1.5 rounded border border-black/15 px-2.5 py-1 text-sm hover:border-accent hover:text-accent"
            @click="restore(row)"
    ...
    <div v-if="hiding" class="fixed inset-0 z-40 flex items-center justify-center bg-black/40 px-4">
      <div class="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
        ...
        <p class="mt-1 truncate text-xs text-black/45">{{ hiding.body }}</p>

        <textarea
          v-model="reason" rows="3" maxlength="500" autofocus
          class="mt-3 w-full rounded border border-black/15 px-3 py-2 text-sm outline-none focus:border-accent"
          placeholder="npr. uvredljiv sadržaj, spam, nije o pjesmi"
        />

        <div class="mt-4 flex justify-end gap-2 text-sm">
          <button class="rounded px-3 py-1.5 text-black/60 hover:text-accent" @click="hiding = null">Odustani</button>
          <button
